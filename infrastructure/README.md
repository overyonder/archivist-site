...
The Resend API key and webhook signing secret are supplied as
`TF_VAR_resend_api_key` and `TF_VAR_resend_webhook_secret`. Their canonical
encrypted copies are the `archivist_resend_api_key` and
`archivist_resend_webhook_secret` SOPS entries.
